...
try{
	...
	submenu.addEventListener('touchstart', clickdown);
	submenu.addEventListener('touchend', clickup);
}
catch(e){ 
	submenu.addEventListener('mousedown', clickdown);
	submenu.addEventListener('mouseup', clickup);
}
...
function posRangeValue(){
	...
	var widthBlock = (ALevel.clientWidth - rangeValue.clientWidth) / 100;
